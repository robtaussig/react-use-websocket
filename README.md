...
#### Messages received from SocketIO
SocketIO sends messages as a string, and in a format that isn't JSON-parsable. One example is:
```
"42["Action",{"key":"value"}]"
```
Other messages might come just as stringified integer (often a response to being pinged). Regardless, an important principle of this library is to minimize the level of abstractions, and as such all messages are passed unparsed. A simple function to convert the message to JSON data might be:
```js
const parseMessage = message => {
  if (message) {
    //Capture all values after initial series of integers
    const regexPayload = /^\d+(\[.*\])$/.exec(message.data);
    if (regexPayload) {
      const result = regexPayload[1];
      //If no result, then the message is just an integer, which is auto-generated by socketIO backend
      if (result) {
        //Returns a tuple, with the first item being the action, and second the payload
        return JSON.parse(result);
      } 
    }
    //Message exists but is not relevant
    return '';
  }
